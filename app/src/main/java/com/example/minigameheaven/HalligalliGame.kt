...
20 12 12 8 4
    ...
    * */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var card  = findViewById<TextView>(R.id.player1_cardNum)

        HalligalliGlobal.cardNums.add(R.id.player1_cardNum)
        HalligalliGlobal.cardNums.add(R.id.player2_cardNum)
        HalligalliGlobal.cardNums.add(R.id.player3_cardNum)
        HalligalliGlobal.cardNums.add(R.id.player4_cardNum)

        //플레이어 4명
        var players = ArrayList<HalligalliPlayer>()
        for(i in 1..playerNum) {
            //플레이어 인원수만큼 플레이어 생성
            players.add(HalligalliPlayer())
        }
        for(i in 0..(players.size-1)){
            //플레이어 배열 돌며 실행할 코드
            findViewById<TextView>(HalligalliGlobal.cardNums.get(i)).setText(""+players.get(i).getCardNum())
        }
